...
class Speaker:
    def __init__(self, name):
        ...
        self.images = {"default": self.image_default, "happy": self.image_happy, "serious": self.image_serious,
                       "concerned": self.image_concerned, "fierce": self.image_fierce,
                       "surprised": self.image_surprised}

    def import_image(self, mood):  # imports and scales the speaker's image for a specific mood
        try:
            image = pygame.image.load(f"graphics/pilots/{self.name}/{mood}.png").convert_alpha()
        except:
    ...
    def update(self):
        ...
        try:
            self.image = self.images[self.mood]
        except:
            self.image = self.image_default
            if debug_mode == True: print("Error: unable to correctly assign speaker's image based on mood for",
                                         self.name, self.mood)
...
class Dialogue:
    def __init__(self):
        ...
        self.text_rect = pygame.Rect((screen_width * 0.2, screen_height * 0.75),
                                     (screen_width * 0.6, screen_height * 0.15))
        ...
        self.speaker_right_rect = pygame.Rect((screen_width * 0.8, screen_height * 0.1),
                                              (screen_width * 0.36, screen_height * 0.8))
    ...
    def make_choice(self, option_number=1):
        if self.cooldown > 0:
            ...
            if self.conversation_topic == "select_mood" and self.conversation_stage == 2:  # select a mood
                if option_number == 1:
                    self.speaker_right.mood = "default"
                elif option_number == 2:
                    self.speaker_right.mood = "happy"
                elif option_number == 3:
                    self.speaker_right.mood = "serious"
                elif option_number == 4:
                    self.speaker_right.mood = "surprised"
